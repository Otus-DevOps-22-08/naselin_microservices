...
```
4. Подправилен манифест и запущен `ui`-компонент.
5. Проверена работоспособность через проброс портов (`kubectl port-forward ui-deployment-b9c4c5b97-bf7nc 8080:929`).
...
11. Автоматизировано развёртываение `k8s` в YC с помощью `terraform`.
...
6. найти внешний IP-дрес ноды: `kubectl get nodes -o wide` и порт публикации сервиса ui: `kubectl describe service ui -n dev | grep NodePort`.

Проверка работоспособности:
- перейти по ссылке, искользуя найденные выше адрес и порт: http://EXTERNAL-IP:NodePort
